...
<head>
    ...
    <!-- Google Fonts -->
    <link
        href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;1,400&family=Inter:wght@300;400;500;600;700&display=swap"
        rel="stylesheet">
    <style>
        body {
...
<body class="bg-[#F0F3F4] text-slate-800 antialiased min-h-screen flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
            <div class="flex items-center space-x-4">
                <a href="{{ url('/admin') }}" class="flex items-center space-x-3">
                    <div
                        class="w-10 h-10 rounded-full border border-brand-gold/40 flex items-center justify-center bg-brand-dark">
                        <span class="font-serif italic text-brand-gold text-xl font-bold">W</span>
                    </div>
                    <div>
                        <h1 class="font-serif text-xl tracking-wider text-stone-100 font-medium leading-none">WAE WATU
                        </h1>
            ...
            <nav class="flex items-center space-x-1 md:space-x-4 text-xs uppercase tracking-wider font-medium">
                <a href="{{ url('/admin') }}"
                    class="px-3 py-2 rounded-lg transition-colors {{ Request::is('admin') ? 'bg-brand-gold text-brand-dark font-bold' : 'text-stone-300 hover:text-white hover:bg-white/10' }}">
                    Dashboard
                </a>
                ...
                <form action="{{ url('/admin/logout') }}" method="POST" class="inline-block pl-2">
                    @csrf
                    <button type="submit"
                        class="bg-rose-900/60 hover:bg-rose-800 text-rose-200 border border-rose-700/50 px-3 py-1.5 rounded-lg text-[10px] uppercase font-bold tracking-wider transition-colors">
                        🚪 Logout
                    </button>
                </form>
            </nav>
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        @if (session('success'))
            <div
                class="mb-6 bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded-r-xl shadow-sm flex items-center justify-between">
        ...
        @if (session('error'))
            <div
                class="mb-6 bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-xl shadow-sm flex items-center justify-between">
                <div class="flex items-center space-x-3">
