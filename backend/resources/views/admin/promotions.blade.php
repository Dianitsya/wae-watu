...
@section('content')
    <div class="space-y-8">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 class="font-serif text-3xl font-normal text-brand-dark">Kelola Tampilan Iklan Highlight & Banner Promo
                </h2>
                <p class="text-xs text-slate-500 mt-1">Ubah judul headline promo, deskripsi singkat, badge teks, dan foto
                    banner utama yang diiklankan di website.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-8">
            @foreach ($promotions as $promo)
                <div
                    class="bg-white rounded-2xl p-6 shadow-sm border border-stone-200 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    <!-- Promo Image Preview -->
                    <div class="lg:col-span-5 space-y-3">
                        <div class="aspect-[16/9] rounded-xl overflow-hidden bg-stone-200 shadow-sm border border-stone-300">
                            <img id="preview-promo-{{ $promo->id }}" src="{{ $promo->image_url }}"
                                alt="{{ $promo->title }}" class="w-full h-full object-cover">
                        </div>
                    ...
                    <div class="lg:col-span-7">
                        <form action="{{ url('/admin/promotions/' . $promo->id) }}" method="POST"
                            ...
                            <div>
                                <label
                                    class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Judul
                                    Headline Utama (Promotional Title)</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Sub-Deskripsi
                                    Iklan Promo</label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Teks
                                    Badge Emblem</label>
                            ...
                            <div class="space-y-2 bg-stone-50/70 p-3.5 rounded-xl border border-stone-200">
                                ...
                                <details class="text-[11px] text-slate-500 pt-1">
                                    <summary class="cursor-pointer font-medium hover:text-brand-dark">Atau gunakan URL
                                        eksternal (opsional)...</summary>
                                    <div class="mt-2">
                            ...
                            <div class="flex items-center space-x-3 pt-2">
                                <input type="checkbox" id="is_active_{{ $promo->id }}" name="is_active" value="1"
                                    {{ $promo->is_active ? 'checked' : '' }}
                                    class="w-4 h-4 rounded text-brand-dark focus:ring-brand-gold">
                                <label for="is_active_{{ $promo->id }}"
                                    class="text-xs font-semibold text-slate-700 cursor-pointer">Tayangkan banner iklan ini
                                    secara live di website</label>
                            </div>

                            <div class="pt-2 flex justify-end">
                                <button type="submit"
                                    class="bg-brand-dark hover:bg-brand-teal text-white px-6 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-wider transition-all shadow-md">
                                    💾 Simpan Banner Iklan
                                </button>
                            </div>
                        </form>
